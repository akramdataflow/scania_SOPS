...
class App(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top, text="Open XML", command=self.on_open_xml, style="Primary.TButton").pack(side="left", padx=(4,0))
        ttk.Button(top, text="Analyze", command=self.on_analyze, style="Primary.TButton").pack(side="left", padx=(8,0))
        ttk.Button(top, text="Save XML", command=self.on_save_xml, style="Primary.TButton").pack(side="left", padx=(8,0))
        ttk.Button(top, text="Export CSV", command=self.on_export_csv, style="Primary.TButton").pack(side="left", padx=(8,0))
        ttk.Separator(top, orient="vertical").pack(side="left", fill="y", padx=10)
        ttk.Button(top, text="analysecntrl unit", command=self.on_analyse_cntrl_unit, style="Primary.TButton").pack(side="left", padx=(8,0))
        ...
        self.btn_modify_apply = ttk.Button(top, text="Apply", command=self.on_modify_choice, style="Secondary.TButton")
        self.btn_modify_apply.pack(side="left", padx=(6,10))
        ttk.Label(top, text="Controller:").pack(side="left", padx=(4,0))
        ...
        self.ent_vin = ttk.Entry(vin_area, textvariable=self.var_vin, state="readonly", width=28)
        self.ent_vin.pack(side="left", padx=(8,0))
        ttk.Button(vin_area, text="CHANGE VIN", command=self.on_change_vin_dialog, style="Secondary.TButton").pack(side="left", padx=(8,0))
        ttk.Button(vin_area, text="Copy", command=self.on_copy_vin, style="Secondary.TButton").pack(side="left", padx=(8,0))
        ...
        bottom_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=6, padx=8)
        exit_btn  = ttk.Button(bottom_frame, text="Exit",  command=self.on_exit,    style="Exit.TButton")
        clear_btn = ttk.Button(bottom_frame, text="Clear", command=__quick_clear,   style="Clear.TButton")
...
# ---------- actions ----------
        ...
        if unsaved:
            ...
            if resp is True:
                try:
                    ...
                    messagebox.showerror(APP_TITLE, f"Failed to save before exit:\n{e}")
                    return
        self.master.destroy()
    ...
    def _extract_vin(self):
        ...
        try:
            ...
            for e in root.iter():
                try:
                    name = (e.attrib.get("Name") or e.attrib.get("name") or "").strip().lower()
                    val  = (e.attrib.get("Value") or e.attrib.get("value") or "").strip()
                except Exception:
                    continue
                if "vin" in name and val:
                    cand = "".join(c for c in val if c.isalnum()).upper()
                    import re as _re
                    if _re.fullmatch(r"[A-HJ-NPR-Z0-9]{17}", cand):
                        return cand
        except Exception:
            pass
    ...
    def on_read_vin(self):
        if not getattr(self, "xml_path", None):
            messagebox.showinfo(APP_TITLE, "Select an XML file first."); 
            return
        vin = self._extract_vin()
        if vin:
            self.current_vin = vin
            try: self.var_vin.set(vin)
            except Exception: pass
            self._log(f"[INFO] VIN detected: {vin}")
        else:
            self._log("[INFO] VIN not found.")

    def on_copy_vin(self):
        vin = (self.var_vin.get().strip() if hasattr(self, "var_vin") else "") or (self.current_vin or "")
        if not vin:
            messagebox.showinfo(APP_TITLE, "No VIN to copy."); return
        try:
            self.master.clipboard_clear()
            self.master.clipboard_append(vin)
            self._log(f"[VIN] Copied to clipboard: {vin}")
        except Exception:
            self._log("[VIN] Clipboard copy failed.")

    def on_change_vin_dialog(self):
        if not getattr(self, "xml_path", None):
            messagebox.showinfo(APP_TITLE, "Select an XML file first."); 
            return
        cur_vin = self._extract_vin() or ""
        cur_short = self._extract_chassis_short() or ""
        win = tk.Toplevel(self.master); win.title("Change VIN"); win.grab_set()
        frm = ttk.Frame(win, padding=10); frm.pack(fill="both", expand=True)
        ttk.Label(frm, text="VIN (17 chars):").grid(row=0, column=0, sticky="e", padx=(0,6), pady=4)
        var_full = tk.StringVar(value=cur_vin)
        ent_full = ttk.Entry(frm, textvariable=var_full, width=32); ent_full.grid(row=0, column=1, sticky="w", pady=4)

        ttk.Label(frm, text="ChassisNo (short):").grid(row=1, column=0, sticky="e", padx=(0,6), pady=4)
        var_short = tk.StringVar(value=cur_short)
        ent_short = ttk.Entry(frm, textvariable=var_short, width=32); ent_short.grid(row=1, column=1, sticky="w", pady=4)

        info = ttk.Label(frm, text="Both values will be replaced when saving the XML.", foreground="gray")
        info.grid(row=2, column=0, columnspan=2, sticky="w", pady=(2,8))

        def _apply():
            import re as _re
            vf = var_full.get().strip().upper()
            vs = var_short.get().strip()
            vf_norm = "".join(c for c in vf if c.isalnum()).upper()
            if vf and not _re.fullmatch(r"[A-HJ-NPR-Z0-9]{17}", vf_norm):
                messagebox.showwarning(APP_TITLE, "Invalid VIN (must be 17 characters)."); return
            self.pending_vin_full = (vf_norm if vf else None)
            self.pending_vin_short = (vs if vs else None)
            if self.pending_vin_full:
                self.current_vin = self.pending_vin_full
                try: self.var_vin.set(self.current_vin)
                except Exception: pass
            self._log(f"[VIN] Pending change set. VIN={self.pending_vin_full or '-'} | ChassisNo={self.pending_vin_short or '-'}")
            win.destroy()
    ...
    # ---------- ECU analyser ----------
    def _collect_ecus_text_fallback(self):
        rows = []
        try:
            raw = self.xml_path.read_text(encoding="utf-8", errors="ignore")
        except Exception:
            raw = self.xml_path.read_bytes().decode("latin1", errors="ignore")
        import re
        for m in re.finditer(r"<\s*ECU\b[^>]*>", raw, flags=re.IGNORECASE):
            tag = m.group(0)
            def get(names):
                for n in names:
                    mm = re.search(rf'{n}\s*=\s*"([^"]*)"', tag, flags=re.IGNORECASE)
                    if mm: return mm.group(1).strip()
                return ""
            name = get(["EcuName","ECUName","name","DiagnosticGeneration","diagGeneration","DiagGeneration"])
            fam  = get(["EcuFamily","ECUFamily","family","DiagnosticFamily","diagFamily","DiagFamily"])
            num  = get(["CplNo","CPLNo","cpl","Cplno","CompleteNumber","completeNumber","CompNumber"])
            rows.append((name, fam, num))
        return rows
    ...
    def on_analyse_cntrl_unit(self):
        ...
        rows = []
        try:
            rows = self._collect_ecus()
        except Exception as e:
            self._log(f"[ECU] parse error: {e}")
        if not rows:
            self._log("[ECU] Structured parse found 0 items. Trying text fallback...")
            try:
                rows = self._collect_ecus_text_fallback()
            except Exception as e2:
                self._log(f"[ECU] Fallback parse error: {e2}")
                rows = []
    ...
    def _show_ecu_popup(self, rows):
        ...
        try:
            for r in rows:
                ...
                if not isinstance(r, (list, tuple)): 
                    r = (str(r), "", "")
                if len(r) < 3:
                    r = tuple(list(r) + [""]*(3-len(r)))
                vals = [str(x or "") for x in r[:3]]
                tree.insert("", "end", values=vals)
                added += 1
        except Exception as e:
            self._log(f"[ECU] UI populate error: {e}")
        ...
        def _save_csv():
            ...
            path = filedialog.asksaveasfilename(title="Save ECU list as CSV",
    ...
    def on_save_xml(self) -> None:
        ...
        try:
            ...
            if getattr(self, 'pending_vin_full', None) or getattr(self, 'pending_vin_short', None):
                ...
                for e2 in root.iter():
                    if not getattr(e2, 'attrib', None):
                        continue
                    n = (e2.attrib.get('Name') or e2.attrib.get('name') or '').strip().lower()
                    if self.pending_vin_full and 'vin' in n:
                        if 'Value' in e2.attrib: e2.attrib['Value'] = self.pending_vin_full
                        if 'value' in e2.attrib: e2.attrib['value'] = self.pending_vin_full
                        rep_full += 1
                    if self.pending_vin_short and 'chassis' in n:
                        if 'Value' in e2.attrib: e2.attrib['Value'] = self.pending_vin_short
                        if 'value' in e2.attrib: e2.attrib['value'] = self.pending_vin_short
                        rep_short += 1
